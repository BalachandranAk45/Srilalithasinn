...
const ReportPage = () => {
  ...
  const handleSearch = () => {
    fetchReport(1); // reset to first page on search
  };
  ...
  return (
    <Box p={8}>
      ...
      <SimpleGrid columns={{ base: 1 }} spacing={4} display={{ base: "grid", md: "none" }} mt={2}>
        {bookings.map((b) => (
          <Card key={b.booking_id} bg={cardBg} shadow="md" borderRadius="xl">
            <CardBody>
              <VStack align="start" spacing={3}>
                <Text fontWeight="600">
                  {b.name} ({b.mobile})
                </Text>
                <Text fontSize="sm">Room: {b.room_no}</Text>
                <Text fontSize="sm">Check-In: {b.checkIn.toLocaleDateString()}</Text>
                <Text fontSize="sm">Check-Out: {b.checkOut.toLocaleDateString()}</Text>
